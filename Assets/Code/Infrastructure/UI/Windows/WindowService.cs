...
namespace Code.Infrastructure.UI.Windows
{
    public class WindowService : IWindowService
    {
        ...
        private readonly Dictionary<Type, Action> _bindings = new();
        private readonly Dictionary<Type, WindowInstance> _instances = new();
        private readonly Dictionary<Type, string> _prefabKeys = new();
        ...
        public void Open<TView>(bool onTop = false) where TView : MonoBehaviour, IWindowView
        {
            ...
            if (_bindings.TryGetValue(key, out Action createRegisteredWindow))
                createRegisteredWindow?.Invoke();
            ...
            where TModel : class, IWindowModel
        {
            ...
            try
            {
                ...
                {
                    Debug.LogWarning(
                        $"[WindowService] Failed to load prefab '{prefabKey}' for window {typeof(TView).Name}");
                    return;
                }
                ...
                ((IWindowPresenter<TModel, TView>)result.Presenter).Initialize((TModel)result.Model,
                    (TView)result.View);
